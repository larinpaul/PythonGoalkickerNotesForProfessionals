...
# # primarily used for referencing items by keys., they are not sorted.
...
# otherdict = {'key1': 'value1', 'key2': 'value2'}
# yetanotherdict = {'key3': 'value3', 'key4': 'value4'}
# d = {**otherdict}
# # also updates the shallow copy with the c\ontents of the yetanotherdict.
...
# # built-int class: dict()
# d = dict() # empty dict
# d = dict(key='value') # explicit keyword arguments
# d = dict([('key', 'value')]) # passing a list of key/value pairs
...
# # It is also possible to add list and dictionary as a value:
# d['new_list'] = [1, 2, 3]
...
# # One common pitfall when using dictionaries is to access a non-existent. This typically results in a KeyError
# # exception
# mydict = {}
# mydict['not there']
...
# # One way to avoid key errors is to use the dict.get method, which allows ou to specify a default value to return in
...
# # after you check, creatinga race condition where the exception can still be thrown.
...
# # If you use a fictionary as an iterator (e.g. in a for statement), it traverses the keys of the dictionary. FOr example:
# d = {'a': 1, 'b': 2, 'c': 3}
# for key in d:
#     print(key, d[key])
# # c3
...
# # The items() method can be used to loop over both the key and value simultaneusly:
...
# # Here, the methods keys(), values() and items() return lists, and there are thre extra methods iterkeys()
# # itervalues() and iteritems() to return iterators.


# ## Section 19.4: Dictionary with default values

# # Available in thee standard library as defaultfict
...
# # [*] Python Cookbook, 3rd edition, by David Bezley and Brian K. Jones (O'Reilly). Copyright 2013 David Beazley and Brian
...
# dict(chain(fish.item(), dog.items()))
# # {'hands': 'paws', 'color': 'red', 'name': 'Clifford', 'special': 'gills'}
...
# # of teh Python 2 iterkeys(), itervalues(),and iteritems() methods. These objects can be used like lists for the
...
# # The string "Hello" in this example is called a key. It is used t lookup a value in the dict by placing the key in
# # square brackets.

# # The number 1234 is seen after the respective colon in thedict definition. This is called the value that "Hello" maps
# # to in this dict.

# # Looking up a value like this with a key that does not exist will raise a KeyError exception, halting execution if
# # uncaight. If we want to access a value without risking a KeyError, we can use the dictionary.get method. By
# # default if the key does not exist, the method will return None. We can pass it a second value to return instead of
# # None in the even of a failed lookup.
...
# ## Section 19.8: Creating a dictioanry
...
# # There isn o particular order for the keys.

# # Creating and populatng it with values
# stock = {'eggs': 5, 'milk': 2}

# # Orcreating an empty dictionary
# dictionary = {}
...
# # Use list.append() metod to add new elements to the values list
# mydict['a'].append(4) # => {'a': [1, 2, 3, 4], 'b': ['one', 'two', 'three']}
# mydict['b'].apped('four') # => {'a': [1, 2, 3, 4], 'b': ['one', 'two', 'three', 'four']}
...
# # You can create an ordered dictionary which willfollow a determined order when iterating over the keys in the
# # dictionary.

# # Ose OrderedDict from the collectons module. This will always return the dictionary lements in the original
...
# # Like lists ad tuples, you can include a trailing comma in yourdictionary.
...
# # Dictionaries map keysto values.
# car = {}
# car["wheels"] = 4
# car["color"] = "Red"
# car["model"] = "Corvetter"
...
# # This would print out "Litte Red Corvetter!"
...
# Given a dictioanry such as the one shown above, where there is a list representing a set of values to explore for the
